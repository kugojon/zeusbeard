...
class AM_RevSlider_Block_Slider_Preview extends Mage_Core_Block_Template implements Mage_Widget_Block_Interface{
    ...
    public function renderJs($slider){
        ...
        /* if ($slider->getData('using_jquery') == 'true'){
            $scripts[] = Mage::getBaseUrl('js',array('_secure'=>true)).'am/extensions/jquery/jquery-1.9.1.min.js';
        } */
        $html .= '<script type="text/javascript">delete window["console"];</script>';
        $scripts[] = Mage::getBaseUrl('js',array('_secure'=>true)).'am/revslider/rs-plugin/js/jquery.themepunch.tools.min.js?v=4.6.0';
        $scripts[] = Mage::getBaseUrl('js',array('_secure'=>true)).'am/revslider/rs-plugin/js/jquery.themepunch.revolution.min.js?v=4.6.0';
        foreach ($scripts as $script){
            $html .= "<script type='text/javascript' src='{$script}'></script>\r\n";
        }
        $styles = array();
        $styles[] = Mage::getBaseUrl('js',array('_secure'=>true)).'am/revslider/rs-plugin/css/settings.css?v=4.6.0';
        $styles[] = $this->getUrl('revslider/index/getCssCaptions', array('id' => $slider->getId(),'_secure'=>true));
