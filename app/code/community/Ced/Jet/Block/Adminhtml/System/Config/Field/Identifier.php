...
class Ced_Jet_Block_Adminhtml_System_Config_Field_Identifier extends Mage_Adminhtml_Block_System_Config_Form_Field_Array_Abstract
{
    ...
    protected function _prepareToRender()
    {
        ...
        $this->addColumn(
            'identifier', array(
            'label' => Mage::helper('jet')->__('Identifier'),
            //  'renderer' => $this->_getIdentifierRenderer(),
            'style' => 'width:60px',
            )
        );
        $this->addColumn(
    ...
    protected function _prepareArrayRow(Varien_Object $row)
    {
        ...
        $row->setData(
            'option_extra_attr_' . $this->_getIdentifierRenderer()
                ->calcOptionHash($row->getData('identifier')),
            'selected="selected"'
        );
     ...
     */
    protected function _getIdentifierRenderer()
    {
        if (!$this->_regionRenderer) {
            $this->_regionRenderer = $this->getLayout()->createBlock(
                'jet/adminhtml_system_config_field_identifier_default', '',
    ...
    protected function _getMagentoAttributeRenderer()
    {
        if (!$this->_methodRenderer) {
            $this->_methodRenderer = $this->getLayout()->createBlock(
                'jet/adminhtml_system_config_field_identifier_magentoattribute', '',
                array('is_render_to_js_template' => true)
            );
