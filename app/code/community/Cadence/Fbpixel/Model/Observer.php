...
Class Cadence_Fbpixel_Model_Observer
{
    ...
    public function onSalesQuoteProductAddAfter(Varien_Event_Observer $obs)
    {
        ...
        foreach ($items as $item) {
            ...
            $candidates['content_name'] = $item->getName();

            $product = Mage::getModel('catalog/product')->load($item['product_id']);

            if($item->getProduct()->getFinalPrice()){
                $candidates['value'] += $item->getProduct()->getFinalPrice() * $item->getProduct()->getQty();
            }else{
                $candidates['value'] += $product->getPrice() * $item->getProduct()->getQty();
            }
            
            $cats = $product->getCategoryIds();
            foreach ($cats as $category_id) {
                $_cat = Mage::getModel('catalog/category')->setStoreId(Mage::app()->getStore()->getId())->load($category_id);
                    $candidates['content_category'] = $_cat->getName();            
            }
        }
        $data = array(
            ...
            'content_name'=> $candidates['content_name'],
            'content_category'=>$candidates['content_category']
        );
