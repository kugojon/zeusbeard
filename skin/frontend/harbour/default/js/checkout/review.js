...
OrderReviewController.prototype = {
    ...
    _submitOrder : function()
    {
        if (this._validateShipping()) {
            ...
            confirm('Please specify a shipping method.');
        }
        this._updateOrderSubmit(true);
    },
    ...
    {
        var shippingMethodClass = '.paypal-express-review #shipping_method_form #shipping_method';
        if (jQuery(shippingMethodClass).length == 0 ||
            (jQuery(shippingMethodClass).length > 0 && !jQuery(shippingMethodClass).val())) {
            return false;
        }
        return true;
    }
