...
final class StoneEdge_MagentoImport {
	...
	private static function writeOrderItem(DOMElement $nd, Mage_Sales_Model_Order_Item $item, 
        ...
        $_item_name = $item->getData('name');
        
        if (!$isBundleItem && is_array($opt)) {
            if(isset($opt['attributes_info']) && is_array($opt['attributes_info'])) {
                $_options = '';
                // is child of configurable product
                foreach ($opt['attributes_info'] as $att) {
                    $_options .= ', '.$att['label'].': '.$att['value'];
                }
                $_item_name .= $_options;
            }
		}
		
		self::xmlAppend("Name", $_item_name, $nd, $xd);
	...
	private static function writeOrderTotals(DOMElement $ndTot, Mage_Sales_Model_Order $order, DOMDocument $xd) {	
		...
		/*if ($order->getData('base_money_for_points') < 0) {
			$ndDiscount = $xd->createElement("RewardPoints");
				self::xmlAppend('Total', abs($order->getData('base_money_for_points')), $ndDiscount, $xd);
		...
		if ($order->getData('base_money_for_points') < 0) {
			$ndDiscount = $xd->createElement("Discount");
				self::xmlAppend('Amount', abs($order->getData('base_money_for_points')), $ndDiscount, $xd);
				self::xmlAppend('Description', 'Reward Points Redemption', $ndDiscount, $xd);
			$ndTot->appendChild($ndDiscount);
		}
        if ($order->getData('base_aw_giftwrap_amount') > 0) {
			$ndSurcharge = $xd->createElement("Surcharge");
				self::xmlAppend('Total', abs($order->getData('base_aw_giftwrap_amount')), $ndSurcharge, $xd);
				self::xmlAppend('Description', 'Gift Wrap', $ndSurcharge, $xd);
			$ndTot->appendChild($ndSurcharge);
		}
        
		//self::writeDiscounts($ndTot, $order, $xd); 
	}
	...
	private static function writeDiscounts(DOMElement $ndOrd, Mage_Sales_Model_Order $order, DOMDocument $xd) {
		// 08/11/15:RJG Look up line item discounts and roll into order
		// As of 1.9, discounts include coupons
		$orderItems = $order->getItemsCollection();
		$totalLineItemDiscountAmount = 0;
		foreach ($orderItems as $item){
			$lineItemDiscountAmount = $item->getDiscountAmount();
			$totalLineItemDiscountAmount += $lineItemDiscountAmount;
			$lineItemDiscountDescription = $item->getDiscountName();
			if ($lineItemDiscountAmount != '') {
				if (self::$_debug) { echo "Line Item Discount detected $lineItemDiscountDescription:$lineItemDiscountAmount<br>";}
			}
			//if ($lineItemDiscountAmount != '') {
				//if (self::$_debug) { echo "Line Item Discount detected $lineItemDiscountDescription:$lineItemDiscountAmount<br>";}
				//$ndDiscount = $xd->createElement("Discount");
				//self::xmlAppend('Description', $lineItemDiscountDescription, $ndDiscount, $xd);
				//self::xmlAppend('Amount', $lineItemDiscountAmount, $ndDiscount, $xd);		
				//$ndOrd->appendChild($ndDiscount);
			//}
			if (self::$_debug) { echo "Total of Line Item Discounts is $totalLineItemDiscountAmount<br>";}
		}
			if ($totalLineItemDiscountAmount != 0) {
				$ndDiscount = $xd->createElement("Discount");
				self::xmlAppend('Description', "Website Discount", $ndDiscount, $xd);
				self::xmlAppend('Amount', $totalLineItemDiscountAmount, $ndDiscount, $xd);		
				$ndOrd->appendChild($ndDiscount);
			}
	}
